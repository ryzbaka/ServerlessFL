const leveljs = window.leveljs 
const levelup = window.levelup
...
async function loadDataset(){
    ...
    showMessage("reading training dataset")
...
function train(nn,epochs,training_X,training_y,testing_X,testing_y){
    ...
    for(let epoch=0;epoch<epochs;epoch++){
        ...
        showMessage(`Epoch: ${epoch+1} training_loss: ${tf.mean(tf.tensor(train_loss)).arraySync()}`)
    }
...
async function normalMain(){
    ...
    showMessage("=== RUNNING NORMAL LEARNING TRAINING LOOP===")
...
async function loadFederatedDataset(index){
    ...
    let testing_X = testingData.iloc({columns:[`0:${testingData.shape[1]-1}`],rows:[`0:${testingData.shape[0]-1}`]})
    let testing_y = testingData.iloc({columns:[`${testingData.shape[1]-1}:`],rows:[`0:${testingData.shape[0]-1}`]})
    showMessage(`fed_training_data${index+1}.csv - fed_validation_data${index+1}.csv - fed_testing_data${index+1}.csv`)
    return [[training_X,training_y],[validation_X,validation_y],[testing_X,testing_y]] 
}
class FedAvg{
    ...
    initialize(newId,isServer){
        if(this.peer){
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Client already initialized!',
            })
        }
        else{
            if(newId){
    ...
    toJSON(){
        if(this.peer){
            const peerObject = {
                ...
                "peer-status": this.peerStatus,
                "peer-designation": this.peerDesignation
            }
            return JSON.stringify(peerObject)
        }
    }
}

const federationTable = document.querySelector("#federation_table_body")

function addClientRow(clientName, clientStatus, clientDesignation, connectionStatus){
    const row = document.createElement("tr")
    row.id = `${clientName}-row`
    const clientNameEntry = document.createElement("td")
    const clientStatusEntry = document.createElement("td")
    const clientDesignationEntry = document.createElement("td")
    const clientConnectionStatusEntry = document.createElement("td")
    clientNameEntry.id = `${clientName}-name`
    clientNameEntry.innerText = clientName
    clientStatusEntry.id = `${clientName}-status`
    clientStatusEntry.innerText = clientStatus
    clientDesignationEntry.id = `${clientName}-designation`
    clientDesignationEntry.innerText = clientDesignation
    clientConnectionStatusEntry.id = `${clientName}-connection`
    clientConnectionStatusEntry.innerText = connectionStatus
    row.appendChild(clientNameEntry)
    row.appendChild(clientStatusEntry)
    row.appendChild(clientDesignationEntry)
    row.appendChild(clientConnectionStatusEntry)
    federationTable.appendChild(row)
}

function removeClientRow(clientName){
    const row = document.getElementById(`${clientName}-row`)
    federationTable.removeChild(row)
}

function rowValueUpdate(clientName,column,value){
    //columns can be : name, status, designation or connection
